...
def train(args):
    ...
    min_lr = [args.min_lr, 2 * args.min_lr]
    ...
    optimizer = optim.SGD([{'params': weights, 'lr': args.min_lr, 'weight_decay': 0.0005},
                           {'params': biases, 'lr': args.min_lr * 2}],
    ...
    scheduler = optim.lr_scheduler.CyclicLR(optimizer, base_lr=min_lr, max_lr=max_lr,
                                            step_size_up=steps_per_epoch * 2,
                                            last_epoch=((current_epoch - 1) * steps_per_epoch)
                                            if current_epoch != 0 else -1)
